...
## 📐 The Philosophy of Financial Data Geometry {#financial-data-geometry}
...
# 1. Initialize with automated provider (yfinance)
generator = DatasetGenerator(data_source='yfinance')

# 2. Run the pipeline
# This automatically handles fetching, cleaning, indicators, and TBM labels
X, y = generator.run(
...
# Assume 'my_df' is a MultiIndex DataFrame with OHLCV columns
...
X, y = generator.run(custom_data=my_df)
```
</details>
...
# analysis_data contains the 't1' (label end dates) generated by the TBM engine
# which is essential to identify the overlap periods.
analysis_data = generator.analysis_data

X_train, y_train, X_test, y_test = purged_train_test_split(
    X, y,
    t1=analysis_data['t1'],
    date_level='Date',    # Level of the MultiIndex used for time
    test_size=0.2,
    pct_embargo=0.01      # 1% embargo to handle long-memory effects
)
